...
object(this)
  ...
  method bounding_box : point * point =
    (pos, ((fst pos)) +. side, (snd pos) +. side)

  method center : point =
  (pos, ((fst pos)) +. (0.5 *. side), (snd pos) +. (0.5 *. side))
...
let s_list = [new rect (1.0, 1.0) 4.0 5.0;
              new circle (0.0, -4.0) 10.0;
              new square (-3.0, -2.5) 4.3];;
